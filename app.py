...
# ✅ CRITIQUE: st.set_page_config() DOIT être la PREMIÈRE commande Streamlit
st.set_page_config(
...
# ✅ Vérification session existante (UNE SEULE FOIS)
try:
...
def analyze_document(prompt: str, user: dict, entreprise_id: str) -> tuple:
    ...
    try:
        # Récupérer les projets antérieurs
...
if not st.session_state.logged_in:
    ...
    with tab2:
        st.info("Formulaire d'inscription à implémenter via forms.signup_form()")
...
# ========================================
    ...
    if profile_data:
        ...
        for projet in profile_data.get("projets", []):
...
# ========================================
    ...
    with st.sidebar:
        ...
        st.markdown(f"### {user.get('nom_entreprise', 'Entreprise')}")
    ...
    with tab2:
        ...
        with st.form("update_profile_form"):
            ...
            with col1:
                ...
                options_specialites = ["Électricité Résidentiel", "Électricité Commerciale", "Électricité Industrielle"]
                current_specialites = user.get('specialites', []) or []
            ...
            with col2:
                ...
                province = st.selectbox("Province *", ["Québec", "Ontario"], index=0)
            ...
            with col4:
                ...
                pwd_confirm = st.text_input("🔒 Confirmer avec mot de passe *", type="password")
            ...
            if st.form_submit_button("💾 Sauvegarder les modifications", use_container_width=False):
                ...
                elif not all([nom_entreprise, specialites, rue, ville, code_postal, contact_nom, contact_telephone]):
...
{text}
                    ...
                    # Analyse avec comparaison projets antérieurs
